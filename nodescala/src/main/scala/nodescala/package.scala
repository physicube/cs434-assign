...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      val futureList = Future.sequence(fs)
      futureList.onComplete {
        case Failure(exception) => p.failure(exception)
        case Success(list) =>
          if (list.forall(elem => elem match {
            case Success(_) => true
            case Failure(_) => false
          })) p.success(list)
          else p.failure(new Throwable("Some of futures failed in fs."))
      }
      p.future
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val calcellationTokenSource = CancellationTokenSource()
      f(calcellationTokenSource.cancellationToken)
      calcellationTokenSource
    }
