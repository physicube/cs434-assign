...
abstract class TweetSet extends TweetSetInterface {
  ...
  def descendingByRetweet: TweetList = {
    try {
      val removedTweetList = this.remove(mostRetweeted)
      new Cons(mostRetweeted, removedTweetList.descendingByRetweet)
    }
    catch {
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val filteredSet = if (p(elem)) acc.incl(elem) else acc

    left.filterAcc(p, right.filterAcc(p, filteredSet))
  }
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    val currentMostRetweeted = if (acc.retweets > elem.retweets) acc else elem
    left.mostRetweetedAcc(right.mostRetweetedAcc(currentMostRetweeted))
  }
