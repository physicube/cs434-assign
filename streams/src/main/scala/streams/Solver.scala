...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] = {
    val newNeighbors = initial.flatMap(x => newNeighborsOnly(neighborsWithHistory(x._1, x._2), explored)).distinct
    if (newNeighbors.isEmpty) 
      initial.reverse 
    else 
      from(newNeighbors #::: initial, explored ++ newNeighbors.map(_._1))
  }
